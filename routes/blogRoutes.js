...
// === POST: Add New Blog with Image (No Token Required) ===

router.post("/", upload.single("image"), async (req, res) => {  
    const { title, summary, content } = req.body;  
    const imagePath = req.file ? `/uploads/blogs/${req.file.filename}` : null;
 try { 
    await db.query("INSERT INTO blogs (title, summary, content, image) VALUES (?, ?, ?, ?)",
...
router.get("/:id", async (req, res) => {
    try {
     const [results] = await db.query("SELECT * FROM blogs WHERE id = ?", 
        [req.params.id]); 
